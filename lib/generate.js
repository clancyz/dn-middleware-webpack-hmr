...
const applyPluginsToUserPath = require('./applyPluginsToUserPath');
...
async function handleOpts (ctx, opts) {
  ...
  if (opts.babel && opts.babel.plugins) {
    opts.babel.plugins = applyPluginsToUserPath(ctx, opts.babel.plugins, 'babel-plugin');
  }

  if (opts.extraPostCSSPlugins) {
    opts.extraPostCSSPlugins = applyPluginsToUserPath(ctx, opts.extraPostCSSPlugins, 'postcss');
  }
...
async function generate (ctx, opts) {
  ...
  let wpConfig = {
    ...
    resolveLoader: defaultResolveOptions,
    output: {
