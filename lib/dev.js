...
async function dev({
  ...
  choosePort(port || DEFAULT_PORT)
    .then(async port => {
      ...
      // if map local(use charles,fiddler etc.), we need
      // change socket server to local ip
      // this needs modification @webpackConfig 
      if (ctx.env === 'maplocal' && opts.env === 'development') {
        if (urls.lanUrlForTerminal.indexOf('localhost') === -1) {
          webpackConfig.output.publicPath = urls.lanUrlForTerminal;
          webpackConfig.plugins.forEach(p => {
            if (p instanceof webpack.DefinePlugin) {
              p.definitions['process.env']['SOCKET_SERVER'] = JSON.stringify(urls.lanUrlForTerminal);
            }
          });
        } else {
          ctx.console.error('无法找到本地ip进行maplocal');
          return;
        }
